...
function generateEmail(sig: typeof SIGNALS[0]): { subject: string; body: string } {
  const evidenceSummary = sig.evidence
    .map(e => `  • ${e.val} — ${e.label}`)
    .join("\n");

  const subject = `[Action Required] Product Library Update — ${sig.id}: ${sig.title.split("—")[0].trim()}`;

  const body = `Hi Product Guide Team,

I'm writing on behalf of the Pro Intelligence platform to flag an active signal that requires an update to the product library. This is a ${sig.level.charAt(0).toUpperCase() + sig.level.slice(1)}-priority issue currently ${sig.status === "review" ? "under review" : "open"} and impacting the ${sig.stage.replace(/^[^\s]+\s/, "")} stage.

Signal: ${sig.id} — ${sig.title}
Detected: ${sig.date}
...
Please prioritize this update and confirm completion in the platform so the signal can be marked as resolved. If you have questions or need additional context, please reply to this thread or reach out to the intelligence team directly.

Thank you,
Carlisle Pro Intelligence`;
...
type ModalSig = typeof SIGNALS[0];

function ContactModal({ sig, onClose }: { sig: ModalSig; onClose: () => void }) {
  const { subject, body } = generateEmail(sig);
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: 620, maxHeight: "88vh", boxShadow: "0 32px 80px rgba(0,0,0,0.22)", fontFamily: "'Outfit', sans-serif" }}
      >
        {/* Modal header */}
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: "#f0f0f0" }}>
          <div>
            <p className="text-[15px] font-semibold" style={{ color: "#121212" }}>Contact Product Guide Team</p>
            <p className="text-[12px] mt-0.5" style={{ color: "#808488" }}>AI-drafted email · {sig.id}</p>
          </div>
          <button onClick={onClose} className="text-[#808488] hover:text-[#121212] transition-colors">
        ...
        {sent ? (
          <div className="flex flex-col items-center justify-center flex-1 py-16 gap-4">
            <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: "#d6f7da" }}>
            ...
            <p className="text-[15px] font-semibold" style={{ color: "#121212" }}>Email sent to Product Guide Team</p>
            <p className="text-[13px]" style={{ color: "#808488" }}>They'll receive instructions to update the library for {sig.id}.</p>
            <button onClick={onClose} className="mt-2 px-5 py-2 rounded-[8px] text-[13px] font-medium" style={{ background: "#f0f0f0", color: "#404040" }}>Close</button>
          </div>
        ) : (
          <>
            {/* To / Subject */}
            <div className="px-6 pt-4 pb-2 flex flex-col gap-2 border-b" style={{ borderColor: "#f0f0f0" }}>
              <div className="flex items-center gap-3">
                <span className="text-[12px] w-14 shrink-0" style={{ color: "#808488" }}>To</span>
                <span className="text-[13px] px-2.5 py-1 rounded-[6px]" style={{ background: "#f5f5f5", color: "#404040" }}>product-guide-team@carlisle.com</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-[12px] w-14 shrink-0" style={{ color: "#808488" }}>Subject</span>
                <span className="text-[13px] truncate flex-1" style={{ color: "#121212" }}>{subject}</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-[4px]" style={{ background: "rgba(0,57,201,0.08)", color: "#0039c9" }}>✦ AI-generated · editable</span>
              </div>
              <textarea
                value={editedBody}
                onChange={e => setEditedBody(e.target.value)}
                className="w-full text-[13px] leading-relaxed outline-none resize-none bg-transparent"
                style={{ color: "#404040", minHeight: 320 }}
              />
            </div>

            {/* Footer */}
            <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: "#f0f0f0" }}>
              <button onClick={onClose} className="text-[13px] font-medium transition-opacity hover:opacity-70" style={{ color: "#808488" }}>Cancel</button>
              <button
                onClick={handleSend}
                disabled={sending}
                className="flex items-center gap-2 px-5 py-2 rounded-[8px] text-[13px] font-medium transition-opacity hover:opacity-80 disabled:opacity-60"
                style={{ background: "#0039c9", color: "white" }}
              >
                {sending ? (
                  <>
...
export default function SignalsPage() {
  ...
  const [contactSig, setContactSig] = useState<ModalSig | null>(null);
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f8]" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      {contactSig && <ContactModal sig={contactSig} onClose={() => setContactSig(null)} />}
      <PageHero
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-4 border-t" style={{ borderColor: "#f0f0f0" }}>
                      ...
                      {sig.libLink ? (
                        <button
                          className="flex items-center gap-1.5 text-[13px] font-medium px-3 py-1.5 rounded-[8px] transition-opacity hover:opacity-80"
                          style={{ background: "#0039c9", color: "white" }}
                          onClick={() => setContactSig(sig)}
                        >
                          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            <line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>
                          </svg>
                          Contact Product Guide Team
                        </button>
                      ) : (
                        <button
                          ...
                          style={{ color: "#0039c9" }}
                          onClick={() => {
                            if      (sig.id === "SIG-002") navigate("/stage-activity?stage=assembly");
